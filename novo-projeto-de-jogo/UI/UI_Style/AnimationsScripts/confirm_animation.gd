...
func _on_confirm_quit_button_down() -> void:
	play("Click")
	is_on_button = true
	is_pressed = true
	waiting_for_release = false  # Resetar quando pressionado

func _on_confirm_quit_button_up() -> void:
...
func _on_confirm_quit_mouse_exited() -> void:
	if is_pressed:
...
func _on_confirm_quit_mouse_entered() -> void:
	play("Focus")
